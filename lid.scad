...
scale([factor, factor, factor]) {
    ...
    scale([factor, factor, factor]) {
        diff() {
        back(pad) right(pad) cuboid(
        [p_width+1,p_height+1,depth+3], 
        
        anchor=TOP+LEFT+FRONT,
        $fn=24
    ) attach(LEFT,RIGHT) left(2*block+1) down(block) tag_this("remove") cuboid([block, 6*block, depth+3], anchor=TOP+LEFT+FRONT,);
        }
    }
